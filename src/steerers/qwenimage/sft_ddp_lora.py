...
def setup_distributed(rank, lock_rank, world_size):
    ...
    dist.init_process_group(
        "nccl", rank=rank, world_size=world_size, device_id=lock_rank
    )
...
def main(_):
    ...
    for n, p in wrapped_model.transformer.named_parameters():
        if "lora" not in n or ("lora" in n and "old" in n):
    ...
    wrapped_model.transformer = DDP(
        ...
        find_unused_parameters=True,  # NOTE: the last txt_mlp of QwenImage does not contribute grad
    )
    ...
    if is_main_process(rank):
        with (
            ...
            wrapped_model.transformer.module.set_adapter("default")
